...
def get_drug_links(browser, url):
    ...
    abc_window = browser.find_element(
        By.CLASS_NAME, 'b-alphabet-body').find_element(
            By.CLASS_NAME, 'head').find_elements(By.TAG_NAME, 'a')
    letters = [a.get_attribute('href') for a in abc_window]
    for a in letters:
        browser.get(a)
...
def get_drug_info(browser, drug_url):
    ...
    try:
        structure_list = browser.find_element(By.CLASS_NAME, 'structure-list')
        drug_dict = {
            'name': None,
            'active_ingredient': [],
        ...
        if 'Действующее вещество' in structure_list.text:
            temp_ingredients = browser.find_element(
                By.XPATH, (
                    "//h2[@id='deistvuyushhee-veshhestvo']"
                    "/following-sibling::div"
                    )).text
            if '+' in temp_ingredients:
                temp_ingredients = temp_ingredients.replace(
                    'IgG+', 'IgG, ').replace(
                        'IgA+', 'IgA, ').replace(
                            'IgM', 'IgM, ')
        ...
        if 'Фармакологическая группа' in structure_list.text:
            try:
                drug_dict['pharmacological_class'] = browser.find_element(
                    By.XPATH, (
                        "//h2[@id='farmakologiceskaya-gruppa']"
                        "/following-sibling::div"
                        )).text
            except NoSuchElementException:
                pass
        if 'Форма выпуска' in structure_list.text:
            try:
                ...
                pass
        if 'Условия отпуска из аптек' in structure_list.text:
            try:
                recipe_data = browser.find_element(
                    By.XPATH, (
                        "//h2[@id='usloviya-otpuska-iz-aptek']"
                        "/following-sibling::div"
                        )).text
